    ...
    <main id="room-edit" class="main-content">
        ...
        <div class="padding-inline-5rem">
            <div class="crud-header flex-center-center">
                <h2>Edit room</h2>
                <form action="{{ route('room.delete', $room->id) }}" method="POST" class="absolute-right">
                    @csrf
                    @method('DELETE')
                ...
                <div class="flex-column padding-block-1rem">
                    ...
                    <div class="three-column-mid-stretch padding-inline-5rem">
                        ...
                        <form action="{{ route('room.update', $room->id) }}" method="POST"
                            ...
                            @method('PATCH')
                            <div class="flex-column padding-block-1rem">
                                <input type="text" class="input-text" required placeholder="Room number"
                                    name="number" value="{{ old('number', $room->room_number) }}">
                                <input type="text" class="input-text" required placeholder="Capacity" name="capacity"
                                    value="{{ old('capacity', $room->capacity) }}">
                                <input type="text" class="input-text" required placeholder="Price per night"
                            ...
                            <div class="flex-column padding-block-1rem">
                                ...
                                <textarea class="input-text flex-grow-1" id="room-edit-description" placeholder="Room description"
                            ...
                            <div class="flex-column padding-block-1rem">
                                <select class="dropdown-select" name="type" required>
                                    @foreach ($roomTypes as $option)
                                        <option class="filter-field-option" value="{{ $option->id }}"
                                            @if ($option->id == old('type', $room->room_type_id)) selected="selected" @endif>
                                            {{ $option->type }}
                                        </option>
                                    @endforeach
                                </select>
                                <select class="dropdown-select" name="view" required>
                                    @foreach ($roomViews as $option)
                                        <option class="filter-field-option" value="{{ $option->id }}"
                                            @if ($option->id == old('view', $room->room_view_id)) selected="selected" @endif>
                                            {{ $option->view }}
                                        </option>
                                    @endforeach
                                </select>
